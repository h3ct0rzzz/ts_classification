...
def variation_coefficient(data: Union[List[float], np.ndarray]) -> float:
    data = np.asarray(data)
    mean = np.mean(data)
    return np.std(data) / mean if mean != 0 else np.nan
...
@feature_function
def quantile(data: Union[List[float], np.ndarray], q: float) -> float:
    data = np.asarray(data)
    return np.quantile(data, q) if len(data) > 0 else np.nan
...
@feature_function
def fft_coefficient(data: Union[List[float], np.ndarray], coeff: int = 0) -> complex:
    data = np.asarray(data)
    fft = np.fft.fft(data)
    return fft[coeff] if len(fft) > coeff else np.nan

@feature_function
def number_crossing_m(data: Union[List[float], np.ndarray], m: float) -> int:
    data = np.asarray(data)
    greater = data > m
...
def permutation_entropy(data: Union[List[float], np.ndarray], order: int = 3, delay: int = 1) -> float:
    ...
    return -np.sum(c[c > 0] * np.log(c[c > 0])) / np.log(float(len(permutations)))
